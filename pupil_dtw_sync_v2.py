...
pairNo_range = (67, 177)  # both values are inclusive!
session_names = ['BG1', 'BG2', 'BG3', 'BG4']
cond = 'unimodális'


# this is the original blacklist, which we reduced quite a bit
...
def data_collect(root_dir, pairNo_range, session_names, pairNos=None):
    ...
    for pairNo in pairNos:
            for session in session_names:
                ...
                mordor_file = glob.glob(
                    f'{root_dir}/**/prepro_pl_data_pair{pairNo}_Mordor_{session}_residual_resampled.hdf5',
                    recursive=True)
    ...
    for pairNo in pairNos:
            for session in session_names:
                gondor_file = glob.glob(
                    f'{root_dir}/**/prepro_pl_data_pair{pairNo}_Gondor_{session}_residual_resampled.hdf5',
                    recursive=True)
...
def compute_dtw(series_one, series_two, series_one_nan, series_two_nan, sample_rate=30, window_size=6, step_size=3):
    ...
    for segment, start in enumerate(window_starts):
        ...
        # if segment_one_nanratio >= cutoff_nan_ratio or segment_two_nanratio >= cutoff_nan_ratio:
        #     distance = np.nan
        # else:
        #     # compute dtw
...
pairNos = extract_condition(all_pairs=pairNo_range, cond=cond)
# collect filenames
paired_files, paired_ids_pairNo, paired_ids_session = data_collect(root_dir, pairNo_range, session_names, pairNos)
...
for paired_file in paired_files:
    ...
    # check for missing files, fill it with nans
...
# Create dataframe
# This seemed like the easier approach. We can use the pivot table function from pandas to create the desired matrix,
# where rows are pair numbers and columns are number of BG-s.
tmp = np.column_stack((paired_ids_pairNo[:, 0], paired_ids_session[:, 0], dtw_mean_results))
dtw_results_df_long = pd.DataFrame(tmp, columns=["Pair Number", "Session", "DTW mean"])  # long format first
dtw_results_df_wide = dtw_results_df_long.pivot(index="Pair Number", columns="Session", values="DTW mean")  # wide format
...
# Saving everything in numpy arrays
if cond == 'alap':
    np.save('pupil_dtw_wide_zscored_resampled_BG_fam_67', dtw_results_df_wide)
    np.save('pupil_dtw_long_zscored_resampled_BG_fam_67', dtw_results_df_long)
    np.save('dtw_window_result_resampled_BG_fam_67', dtw_array)
    np.save('mordor_nan_ratios_windows_resampled_BG_fam_67', nans_array_mordor)
    np.save('gondor_nan_ratios_windows_resampled_BG_fam_67', nans_array_gondor)
if cond == 'ismeretlen':
    np.save('pupil_dtw_wide_zscored_resampled_BG_unfam_67', dtw_results_df_wide)
    np.save('pupil_dtw_long_zscored_resampled_BG_unfam_67', dtw_results_df_long)
    np.save('dtw_window_result_resampled_BG_unfam_67', dtw_array)
    np.save('mordor_nan_ratios_windows_resampled_BG_unfam_67', nans_array_mordor)
    np.save('gondor_nan_ratios_windows_resampled_BG_unfam_67', nans_array_gondor)
if cond == 'unimodális':
    np.save('pupil_dtw_wide_zscored_resampled_BG_uni_67', dtw_results_df_wide)
    np.save('pupil_dtw_long_zscored_resampled_BG_uni_67', dtw_results_df_long)
    np.save('dtw_window_result_resampled_BG_uni_67', dtw_array)
    np.save('mordor_nan_ratios_windows_resampled_BG_uni_67', nans_array_mordor)
    np.save('gondor_nan_ratios_windows_resampled_BG_uni_67', nans_array_gondor)
